...
_TEAM_PROMPTS: dict[str, str] = {
    ...
    "solution_arch": (
        ...
        "- Do deep analysis before deciding.\n"
        "- Extract all available constraints from the requirement first.\n"
        "- Keep the user in the loop by explicitly listing unknowns and asking targeted clarification questions.\n"
        "- Never hide uncertainty: label assumptions clearly and tie each assumption to a risk.\n\n"
        ...
        "1B. UI BENCHMARKING + OSS REFERENCE REVIEW (MANDATORY)\n"
        "   - Review latest UI patterns for similar apps (children/education/gamified productivity where relevant)\n"
        "   - Inspect public open-source repos for implementation patterns and pitfalls\n"
        "   - Summarize what to adopt vs avoid (performance, accessibility, maintainability)\n"
        "   - Cite at least 3 concrete references (repo/project names or URLs when available)\n\n"
        ...
        "KNOWN INPUTS:\n"
        "- [facts explicitly stated by user]\n\n"
        "ASSUMPTIONS (must be explicit):\n"
        "- [A1] [assumption] — Impact if wrong: [impact]\n"
        "- [A2] [assumption] — Impact if wrong: [impact]\n\n"
        "OPEN QUESTIONS FOR USER:\n"
        "- [Q1] [specific question needed to reduce architecture risk]\n"
        "- [Q2] ...\n"
        "(Ask as many as needed; target 8-15 when requirement is ambiguous. If none, write: '- None')\n\n"
        "DECISIONS:\n"
        "- UI Stack: [chosen framework + component lib + state mgmt + styling]\n"
        "- UI References Reviewed: [latest UI trends + OSS repos reviewed + key takeaways]\n"
        ...
        "IMPORTANT: Always include OPEN QUESTIONS FOR USER even when decisions are made, so the user can confirm/adjust architecture early."
    ),
    "api_design": (
